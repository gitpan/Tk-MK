...
## Tk::ResizeButton V1.3 has been used here.
...
BEGIN {
	eval {
		...
		import Tk::ResizeButton;
	};
	if ($@) { # ups, no Tk::ResizeButton, use this one here
		########################################################################
		...
		# Revision: 2.0
		#########################################################################=
		# Note: For space reason all other documentation of Tk::ResizeButton has
		# been removed See Tk::ResizeButton-Pod for details.
		#
...
# 		sub EnterFocus
# 		{
# 			print "reached EnterFocus of ResizeButton\n";
# 		 my $w  = shift;
#  			print "widget is >$w<\n";
...
# 
#     $class->SUPER::ClassInit($window);
# 	$window->bind($class, '<Double-1>', sub { print "defaultdbl1\n"; } );
# 	return $class;
# }
...
sub header 
{
	...
	$this->SUPER::header($cmd, $column, @args);
}
...
opening and closing of columns, taking benefit from TK::ResizeButton
